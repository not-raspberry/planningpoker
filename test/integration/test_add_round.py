...
def test_add_round(game_id, moderator):
    ...
    new_round_empty_name = moderator.post('/game/%s/new_round' % game_id, json={'round_name': ''})
    assert new_round_empty_name.status_code == 400

    new_round = moderator.post('/game/%s/new_round' % game_id, json={'round_name': round_name})
