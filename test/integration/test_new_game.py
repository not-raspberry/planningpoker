...
def test_create_new_game_too_small_cards_set_error(client, cards):
    ...
    too_little_cards = client.post('/new_game', json={'cards': cards, 'moderator_name': 'Tim'})
...
def test_create_2_games(client):
    ...
    game_1 = client.post('/new_game', json={'cards': [1, 3, 5], 'moderator_name': 'Robert'})
    game_2 = client.post('/new_game', json={'cards': [1, 3, 5, 10, '?'], 'moderator_name': 'Bobby'})

    game_1_id = game_1.json()['game_id']
    game_2_id = game_2.json()['game_id']

    new_round_game_1 = client.post('/game/%s/new_round' % game_1_id, json={'round_name': 'round'})
    assert new_round_game_1.status_code == 200
    new_round_game_2 = client.post('/game/%s/new_round' % game_2_id, json={'round_name': 'round'})
